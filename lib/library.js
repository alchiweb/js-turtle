...
window.forward = function (distance, step = false) {
    if (stepByStep && !step) {
        ...
        if(pixelMode) {
            distance = parseInt(distance) * pixelSize;
        }

        imageContext.save();
        centerCoords(imageContext);
        imageContext.beginPath();
        var maxX = pixelMode ? imageContext.canvas.width : imageContext.canvas.width / 2;
        var minX = pixelMode ? 0 :-imageContext.canvas.width / 2;
        var maxY = pixelMode ? imageContext.canvas.height : imageContext.canvas.height / 2;
        var minY = pixelMode ? 0 :-imageContext.canvas.height / 2;
        var x = turtle.pos.x;
        var y = turtle.pos.y;
        while (distance > 0) {
            if (pixelMode)
                imageContext.moveTo(x+ Math.cos(turtle.angle)*turtle.width/2, y - Math.sin(turtle.angle)*turtle.width/2);
            else
            ...
            function xWrap(cutBound, otherBound) {
                ...
                if (pixelMode)
                    imageContext.lineTo(cutBound+ Math.cos(turtle.angle)*turtle.width/2, edgeY - Math.sin(turtle.angle)*turtle.width/2);
                else
            ...
            function yWrap(cutBound, otherBound) {
                ...
                if (pixelMode)
                    imageContext.lineTo(edgeX+ Math.cos(turtle.angle)*turtle.width/2, cutBound - Math.sin(turtle.angle)*turtle.width/2);
                else
            ...
            function noWrap() {
                if (pixelMode)
                    imageContext.lineTo(newX+ Math.cos(turtle.angle)*turtle.width/2, newY - Math.sin(turtle.angle)*turtle.width/2);
                else
...
window.circle = function (radius, colorFill = "", step = false) {
    if (stepByStep && !step) {
        if (!isNaN(radius))
        steps.push(["circle", radius + ",'" + colorFill + "'"]);
    }
    else {
        radius = parseFloat(radius);
        if(pixelMode) {
            radius = parseInt(radius) * pixelSize;
        }

        imageContext.save();
        centerCoords(imageContext);
        imageContext.beginPath();
        let center_x_arc = turtle.pos.x + Math.cos(turtle.angle)*radius;
        let center_y_arc = turtle.pos.y - Math.sin(turtle.angle)*radius;
        imageContext.arc(center_x_arc, center_y_arc, radius, 0, 2 * Math.PI);
        if (turtle.penDown) {
            if (colorFill == "") {
                imageContext.stroke();
            }
            else {
                let old_fill_style = imageContext.fillStyle;
                imageContext.fillStyle = colorFill;
                imageContext.fill();
                imageContext.fillStyle = old_fill_style;
            }
        }
        imageContext.restore();
        drawIf();
...
window.left = function (angle, step = false) {
    if (stepByStep && !step) {
        ...
        if (pixelMode) {
            turtle.angle += degToRad(angle);
            turtle.pos.x += Math.sin(turtle.angle)*pixelSize;
            turtle.pos.y += Math.cos(turtle.angle)*pixelSize;
        }
        else {
            turtle.angle -= degToRad(angle);
        }
...
window.goto = function (x, y, step = false) {
    if (stepByStep && !step) {
        ...
        if (pixelMode)
        {
            x = x *pixelSize;// + turtle.width/2*Math.cos(turtle.angle);
            y = y * pixelSize;// + turtle.width/2*Math.sin(turtle.angle);
        }
        turtle.pos.x = x;
        turtle.pos.y = y;
        drawIf();
...
window.setPixel = function (x, y, color = "", step = false) {
    if (stepByStep && !step) {
        ...
        if (color == "transparent" ) {
            imageContext.clearRect((x) * pixelSize +1, (y) * pixelSize + 1, pixelSize-2, pixelSize-2);
        }
        else {
            imageContext.fillStyle = color === "" ? imageContext.strokeStyle : color;
            imageContext.fillRect((x) * pixelSize +1, (y) * pixelSize + 1, pixelSize-2, pixelSize-2);
        }
...
function initialise() {
    ...
    turtleContext.imageSmoothingEnabled= false;
...
function draw() {
    ...
    if (turtle.visible) {
        var x = pixelMode ? turtle.pos.x + Math.cos(turtle.angle)*turtle.width/2 : turtle.pos.x;
        var y = pixelMode ? turtle.pos.y - Math.sin(turtle.angle)*turtle.width/2 : turtle.pos.y;
        var w = 10;
        var h = 15;
        turtleContext.save();

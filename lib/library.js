...
window.showGrid = function (step, isStep = false) {
    if (stepByStep && !isStep) {
        ...
        goto(12, 10);
        write("(0,0)");
        goto(turtleCanvas.width / 2 - 4, 11);
        write("X");
        goto(14, turtleCanvas.height / 2 - 6);
        write("Y");
...
window.showPixelGrid = function (step, isStep = false) {
    if (stepByStep && !isStep) {
        ...
        stepByStep = false;
        angle(0);
        goto(-turtleCanvas.width / 2 + 16, +turtleCanvas.height / 2 - 10);
        write("(0,0)");
        goto(turtleCanvas.width / 2 - 8, +turtleCanvas.height / 2 - 10);
        write("X");
        goto(-turtleCanvas.width / 2 + 10, -turtleCanvas.height / 2 + 10);
        write("Y");
...
window.setPixel = function (x, y, color = "", step = false) {
    if (stepByStep && !step) {
        ...
        imageContext.fillRect((x) * pixelSize - turtleCanvas.width / 2, (-1 - y) * pixelSize + turtleCanvas.height / 2, pixelSize, pixelSize);
        if (color !== "") {
...
function initialise() {
    ...
    if (msg === null && document?.body !== null) {
        ...
        msg.innerText = "Les coordonnées ont été copiées dans le presse-papier";
    ...
    turtleCanvas.addEventListener('click', function (e) {  // use event argument
        copyTextToClipboard("(x:"+canvasX + ",y:" + canvasY+")");
        if (msg !== null) {

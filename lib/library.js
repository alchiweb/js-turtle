...
window.getPixelColor = function (x, y) {
    return (new RGBColor(getPixelHexColor(x,y))).toSimpleColor();
}
...
function initialise() {
    ...
    turtleCanvas.addEventListener('click', function (e) {  // use event argument
        copyTextToClipboard("("+canvasX + "," + canvasY+") " + getPixelColor(canvasX, canvasY));
        if (msg !== null) {
    ...
    turtleCanvas.addEventListener("mousemove", function (event) {
        ...
        if (co !== null) {
            co.innerHTML = "X:" + canvasX + "&#9;Y:" + canvasY + "&#9;" + getPixelColor(canvasX, canvasY);
        }
    });
